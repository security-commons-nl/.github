// De gedeelde leesversie-build van de commons. Dit is de enige kopie: repo's roepen hem aan via de
// herbruikbare workflow pages-docs.yml, die deze repo uitcheckt en SITE_ROOT op de aanroepende repo
// zet. Wat per repo verschilt, staat in site/config.json van die repo; wat hier staat is de vorm.
//
// Losse kopieen in normen, csir-assessment-tool, applicatiecheck, ai-gebruik-in-beeld en
// Handelingsperspectief zijn er nog en werken zonder SITE_ROOT; die stappen een voor een over.
...
// Zonder SITE_ROOT bouwt de build de repo waar hij zelf in staat (zoals de kopieen deden). Met
// SITE_ROOT bouwt hij een andere repo: daar staan de teksten, de config en dist/, hier de opmaak.
const ROOT = process.env.SITE_ROOT ? resolve(process.env.SITE_ROOT) : join(SITE_DIR, '..');
const CONFIG_DIR = process.env.SITE_ROOT ? join(ROOT, 'site') : SITE_DIR;
...
function buildPage() {
  ...
  // Bij een enkele tab is een tabbalk alleen maar ruis; die repo's tonen gewoon hun README.
  const nav = allTabs.length < 2 ? '' : allTabs
