...
- Werkmap: `X:\SECURITY-COMMONS-NL\` (Windows; gebruik Git Bash voor alle commando's hieronder).
- **Redactiestatuut** `X:\SECURITY-COMMONS-NL\.github\REDACTIESTATUUT.md`: Nederlands; geen persoonsnamen,
...
- **Commits:** Nederlands, één onderwerp, map als prefix (`check: ...`, `paden: ...`). Stage alleen
  ...
  commit-msg-hook: `cp X:/SECURITY-COMMONS-NL/kennisbank/.git/hooks/commit-msg <repo>/.git/hooks/commit-msg`.
...
- **Node 24, Python 3.12+** zijn aanwezig. `X:\TOOLS\pandoc\pandoc.exe` voor HTML uit markdown.
...
- [ ] **Stap 1: Bron van de zelfcheck aanwezig?** Controleer dat er een map is met de broncode van de
  ...
  vragen ergens als data of in componenten). Verwachte plek: `X:\SECURITY-COMMONS-NL\_wachtkamer\check-bron\`.
...
- [ ] **Stap 2: Naam vastgesteld?** Zoek `X:\SECURITY-COMMONS-NL\_wachtkamer\NAAM.txt`. Staat er één woord in,
...
- [ ] **Stap 3: Licentie en maintainer.** Controleer in de bronmap op een `LICENSE`. Is hij niet EUPL-1.2 of
  ontbreekt hij: kopieer `X:\SECURITY-COMMONS-NL\kennisbank\LICENSE` erbij in Taak 1 en noteer in
...
- Per blad minimaal twee chokepoints; elke chokepoint heeft één vraag in de vier velden (claim, toelichting,
  ...
  en uit `X:\SECURITY-COMMONS-NL\kennisbank\security\risicoanalyse-aanvalspaden\aanvalspaden.md`
...
- [ ] **Stap 5: Schrijf `paden.json`** volgens de structuur en regels hierboven: 5 clusters, 18 bladeren,
  ...
  `X:\SECURITY-COMMONS-NL\_wachtkamer\check-bron\...` of, als alleen de HTML er is, extraheer de
  strings met dit script en gebruik ze als bron:

```bash
python - <<'EOF'
import re, sys
t = open(r"X:/SECURITY-COMMONS-NL/_wachtkamer/Gemeentelijke-weerbaarheidscheck-offline.html", encoding="utf-8").read()
...
- Modify: `X:\SECURITY-COMMONS-NL\kennisbank\security\risicoanalyse-aanvalspaden\README.md`,
  `X:\SECURITY-COMMONS-NL\kennisbank\security\risicoanalyse-aanvalspaden\aanvalspaden.md`
...
```bash
cd X:/SECURITY-COMMONS-NL/kennisbank && python tools/build.py
...
```bash
cd X:/SECURITY-COMMONS-NL/security-posture-tool
...
**Bestanden:**
- Modify: `X:\SECURITY-COMMONS-NL\.github\profile\README.md` (projectentabel),
  `X:\SECURITY-COMMONS-NL\.github\REDACTIESTATUUT.md` (tabel bij B1),
  `X:\SECURITY-COMMONS-NL\CLAUDE.md` (projecten + routing),
  `X:\SECURITY-COMMONS-NL\security-commons-nl.github.io\llms.txt` (via de build).
...
```bash
cd X:/SECURITY-COMMONS-NL/.github && git add profile/README.md REDACTIESTATUUT.md && git commit -m "profiel: aanvalspaden-keten toegevoegd; posture-tool als diepte 2" && git push
